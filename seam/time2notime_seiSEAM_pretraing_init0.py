...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        if epoch % 50 == 0:
            file_params1 = "/data/hany/zhangpm/weight/init0model1_pretraing_2shot_test{0}_loss{1}".format(epoch, epoch_loss)
            torch.save(model1, file_params1)

            file_params2 = "/data/hany/zhangpm/weight/init0model2_pretraing_2shot_test{0}_loss{1}".format(epoch, epoch_loss)
            torch.save(model2, file_params2)
            print("model has been stored!")
        if epoch == 999:
            file_params1 = "/data/hany/zhangpm/weight/init0model1_pretrainh_2shot_test{0}_loss{1}".format(epoch, epoch_loss)
            torch.save(model1, file_params1)
